...
/- SabNote:
open Classical gives us access to the Classical library/module. Therefore,
every Veil namespace has access to the Classical library and can use classical
logic.
-/
...
#check Nonempty
/- SabNote:
The Nonempty typeclass is a proposition indicating that `type node` (and any
other type declared with the Veil command `type`) is not empty.
-/
...
/-SabNote:
Why use a class? Because classes synthesize the input t automatically using
typeclass resolution.

The definition of the TotalOrder class is as follows:
class TotalOrder (t : Type) where
  -- relation: total order
  le (x y : t) : Prop
  /-SabNote: relations are propositions in math--x < y is a true/false question -/
  -- axioms
  /-SabNote: total order requires reflexivity, transitivity, antisymmetry, and
  totality.-/
  le_refl       (x : t) : le x x
...
SabNote:
Between is another class with propositions that help with implementing rings.
-/
...
/- `leader n = True` means node `n` believes it is the leader.
  ...
  SabNote: In the Veil manual, they describe relations as predicates over types
  that return Bool or propositions. So, leader is a predicate on the node type
  that indicates whether it is a leader.
  -/
...
SabNote: from digging around, it seems that gen_state calls assemble_state, which
packages all relation predicates into a single `State` type.
-/
#gen_state
...
SabNote: Essentially, the `State` type has two fields and a constructor. The
`deriving` keyword means that the structure will generate an instance of the class
Nonempty.
-/
#print State
...
/- SabNote:
In the initial state of the ring, no nodes are leaders, and no messages are
being sent between nodes.

We use the relations we defined earlier to define the initial state.
-/
...
SabNote:
The requirement here specifies two conditions. First,n and next cannot be the
same. Second, for all Z, if Z is not n and Z is not next, then next is between
n and Z. This second condition asserts that we must have a ring that we traverse
clockwise. It also ensures that next really is the successor to n, as otherwise,
next may not be between n and Z; rather, Z would be between n and next.
If these conditions are met, then n sends a message to next.
-/
action send (n next : node) = {
  ...
  pending n next := True -- n is sending a message to next
}
...
SabNote: I believe this condition would allow us to replace the second condition
in `send` with
  `require n ≠ next ∧ isNext n next`
-/
...
SabNote: the Veil documentation describes safety properties as the main
correctness property that we want to verify.
-/
...
SabNote: invariants must hold in all reachable states and are used in inductive
proofs.
-/
...
SabNote: this command is used to finalize the specification so that we can move
on to verification.
-/
#gen_spec
